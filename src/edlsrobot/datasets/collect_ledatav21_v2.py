...
def collect_and_return_buffer(args, dataset, ros_operator, save_queue, episode_num):
    rate = Rate(args.frame_rate)
    # 初始化机器人基础位置
    ...
    episode_buffer_new = dataset.create_episode_buffer(episode_num)  # 新 buffer
    dataset.episode_buffer = episode_buffer_new
    ...
    while (count < args.max_timesteps) and rclpy.ok() and not joy_key_3:
        ...
        if prev_state is not None:
            ...
            # 调用 add_frame，但用新的 buffer
            dataset.add_frame(frame, epis_idx=episode_num)
            # 添加元素
    ...
    if joy_key_3:
        ...
        #等图像写入完成
        dataset._wait_image_writer()
        frozen_buffer = deepcopy(dataset.episode_buffer)
        # 采集完成 →  放入队列
        save_queue.put((episode_num, frozen_buffer))
        print(f"\033[34mEpisode queued, continue to next collection\033[0m")
...
def main(args):
    ...
    def save_worker(dataset, save_queue):
        while rclpy.ok():
            try:
                item = save_queue.get()  # 阻塞等待，有任务就处理
                if item is None:
                    save_queue.task_done()
                    break
                episode_num, episode_buffer = item
                dataset.save_episode(episode_data=episode_buffer, epis_index=episode_num)
                ...
                save_queue.task_done()
                print("\033[32mEpisode saved in background!\033[0m")
    
    threading.Thread(target=save_worker, args=(dataset, save_queue), daemon=True).start()
    ...
    try:
        ...
        while episode_num < num_episodes and rclpy.ok():
            ...
            print(f"Start to record episode {episode_num}")
            # collect_and_save(args, dataset, ros_operator, voice_engine)  # ros_operator.process()
            # ---------- 采集数据 ----------
            collect_and_return_buffer(args, dataset, ros_operator, save_queue, episode_num)
            # 不直接保存，放入队列
            if joy_key_3:
        ...
        save_queue.join()       # 等待队列中所有任务完成
        save_queue.put(None)    # Sentinel signal
        time.sleep(0.5)
...
def parse_arguments(known=False):
    ...
    # 数据集配置
    parser.add_argument('--root_path', type=str, default=Path.joinpath(ROOT, './All_datas/pour_tea'), help='dataset dir')
    ...
    # 数据采集选项
    parser.add_argument('--resume', type=bool, default=True, help='resume data collect')

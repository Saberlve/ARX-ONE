...
##
from typing import Dict, Union, List, Any, Tuple, Iterator, Set
...
from .common_utils import is_valid_numpy_dtype_string
...
def embed_images(dataset: datasets.Dataset) -> datasets.Dataset:
    ...
    dataset = dataset.map(embed_table_storage, batched=False, num_proc=2) #加速
...
def hf_transform_to_torch(items_dict: Dict[str, Union[torch.Tensor, None]]):
    """Get a transform function that converts items from Hugging Face dataset (pyarrow)
...
def get_episode_data_index(
    episode_dicts: Dict[str, Dict[str, Any]],

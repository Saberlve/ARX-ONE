...
def legacy_load_episodes_stats(local_dir: Path, not_used_keys: list) -> dict:
    episodes_stats = load_jsonlines(local_dir / LEGACY_EPISODES_STATS_PATH)
    items_dict = {}
    for item in sorted(episodes_stats, key=lambda x: x["episode_index"]):
        for key in not_used_keys:
            del item["stats"][key]
        items_dict.update({item["episode_index"]: cast_stats_to_numpy(item["stats"])})
    return items_dict
...
def concat_data_files(paths_to_cat, new_root, chunk_idx, file_idx, image_keys, not_used_key):
    # TODO(rcadene): to save RAM use Dataset.from_parquet(file) and concatenate_datasets
    ## read used cols
    use_cols = pd.read_parquet(paths_to_cat[0], engine="pyarrow").columns
    use_cols = use_cols.to_list()
    for key in not_used_key:
        if key in use_cols:
            use_cols.remove(key)

    dataframes = [pd.read_parquet(file, columns=use_cols) for file in paths_to_cat]
...
def convert_data(root: Path, new_root: Path, data_file_size_in_mb: int, not_used_keys: list):
    data_dir = root / "data"
    ...
    for ep_path in tqdm.tqdm(ep_paths, desc="convert data files"):
        ...
        if paths_to_cat:
            concat_data_files(paths_to_cat, new_root, chunk_idx, file_idx, image_keys, not_used_keys)
...
def convert_episodes_metadata(root, new_root, episodes_metadata, not_used_keys, episodes_video_metadata=None):
    logging.info(f"Converting episodes metadata from {root} to {new_root}")

    episodes_legacy_metadata = legacy_load_episodes(root)
    episodes_stats = legacy_load_episodes_stats(root,not_used_keys )
...
def convert_info(root, new_root, data_file_size_in_mb, video_file_size_in_mb, not_used_keys):
    info = load_info(root)
    ...
    ## add
    for key_ in not_used_keys:
        del info["features"][key_]
...
def convert_dataset(
    ...
    not_used_keys: list[str] | None = None,
):
    ...
    convert_info(root, new_root, data_file_size_in_mb, video_file_size_in_mb, not_used_keys)
    convert_tasks(root, new_root)
    episodes_metadata = convert_data(root, new_root, data_file_size_in_mb, not_used_keys)
    episodes_videos_metadata = convert_videos(root, new_root, video_file_size_in_mb)
    convert_episodes_metadata(root, new_root, episodes_metadata, not_used_keys, episodes_videos_metadata)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--not_used_keys",
        nargs="*",
        # default = ["observation.velocity", "observation.effort", "action_eff"],   #不需要使用的key
        default = [],
        help = "List of keys to ignore when converting dataset.",
    )
...
--repo-id=/home/ubuntu/edlsrobot/repos/ROS2_AC-one_Play/All_datas/pour_tea200 \
--root=/home/ubuntu/edlsrobot/repos/ROS2_AC-one_Play/All_datas \
--push-to-hub=false
...
--repo-id=/home/ubuntu/edlsrobot/repos/ROS2_AC-one_Play/All_datas/test \
--root=/home/ubuntu/edlsrobot/repos/ROS2_AC-one_Play/All_datas \
--push-to-hub=false
'''

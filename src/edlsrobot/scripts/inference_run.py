...
def get_obervations(args, timestep, ros_operator):
    ...
    rate = Rate(args.robot.frame_rate)
...
def inference_process(cfg, ros_operator):
    engine = init_infer_engine(cfg)
    
    action_history = []   # 保存每一步 action
    
    rate = Rate(cfg.robot.frame_rate)
    timestep = 0
    # while timestep < cfg.robot.max_publish_step and ros_proc.is_alive():
    while timestep < 3000 and rclpy.ok():
        obs_dict = get_obervations(cfg, timestep, ros_operator)

        observation = {
            "observation.state": (obs_dict["qpos"].astype(np.float32)),
            "observation.velocity": (obs_dict["qvel"].astype(np.float32)),
            "observation.effort": (obs_dict["effort"].astype(np.float32)),
        ...
        # time.sleep(1)
        robot_action(ros_operator, cfg.robot, action)
...
def main(cfg: InferenceConfig):
    ...
    # 推理
    try:
        inference_process(cfg, ros_operator)
